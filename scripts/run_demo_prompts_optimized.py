"""Run demo prompts v6 — recursive depth-2 with moondream vision.

Uses max_depth=2 so the orchestrator can spawn child RLM loops via
rlm_query(), and moondream for both vision and caption models.
Budget capped at $1 per prompt.

Usage:
    uv run python scripts/run_demo_prompts_optimized.py
    uv run python scripts/run_demo_prompts_optimized.py --prompt 2
    uv run python scripts/run_demo_prompts_optimized.py --prompt 2 4 9
"""
...
def run_prompt(
    ...
    # Detect subtitle generation cost
...
def main():
    parser = argparse.ArgumentParser(description="Run sanjaya demo prompts v6 (depth-2, moondream)")
    parser.add_argument(
        "--prompt",
        type=int,
        nargs="*",
