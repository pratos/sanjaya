...
import { submitDocumentRun, streamEvents } from "./api";
...
export function useDocumentRun() {
  ...
  const startRun = useCallback(
    async (params: {
      ...
      try {
        const { run_id } = await submitDocumentRun({
          document_paths: params.documentPaths,
...
function deriveTokenTotals(events: TraceEvent[]): TokenTotals {
  ...
  const kinds = new Set(["root_response", "sub_llm", "schema_generation", "critic_evaluation"]);
