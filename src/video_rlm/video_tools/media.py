...
def ffprobe_metadata(video_path: str) -> dict:
    ...
    cmd = [
        ...
        "error",
        "-show_entries",
        "format=duration,filename:stream=index,codec_type,width,height,r_frame_rate,avg_frame_rate",
        "-of",
        "json",
        str(path),
    ]
...
def video_duration_seconds(video_path: str) -> float:
    """Return duration in seconds from ffprobe metadata."""
    meta = ffprobe_metadata(video_path)
    duration = meta.get("format", {}).get("duration")
    try:
        return float(duration)
    except (TypeError, ValueError):
        raise MediaToolError(f"Could not parse duration from ffprobe metadata: {duration!r}")
