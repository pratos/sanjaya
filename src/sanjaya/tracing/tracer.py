...
class Tracer:
    ...
    # ── Legacy compat shims ─────────────────────────────────
    # These delegate to generic spans so old code keeps working
    # during migration. Will be removed after Phase 7.

    @contextmanager
    def rlm_completion(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        with self._span("sanjaya.completion", **kwargs) as ctx:
            yield ctx

    @contextmanager
    def sub_llm_call(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        with self._span("sanjaya.llm_call", **kwargs) as ctx:
            yield ctx

    @contextmanager
    def video_completion(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        with self._span("sanjaya.completion", **kwargs) as ctx:
            yield ctx

    @contextmanager
    def video_iteration(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        with self._span("sanjaya.iteration", **kwargs) as ctx:
            yield ctx

    @contextmanager
    def video_orchestrator_call(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        with self._span("sanjaya.orchestrator_call", **kwargs) as ctx:
            yield ctx

    @contextmanager
    def video_code_execution(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        code = kwargs.pop("code", "")
        with self._span("sanjaya.code_execution", code_preview=str(code)[:200], **kwargs) as ctx:
            yield ctx

    @contextmanager
    def video_sub_llm_call(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        with self._span("sanjaya.llm_call", **kwargs) as ctx:
            yield ctx

    @contextmanager
    def video_retrieval(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        with self._span("sanjaya.tool_call", tool_name="list_windows", **kwargs) as ctx:
            yield ctx

    @contextmanager
    def video_clip_extraction(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        with self._span("sanjaya.tool_call", tool_name="extract_clip", **kwargs) as ctx:
            yield ctx

    @contextmanager
    def video_frame_sampling(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        with self._span("sanjaya.tool_call", tool_name="sample_frames", **kwargs) as ctx:
            yield ctx

    @contextmanager
    def video_vision_query(self, **kwargs: Any) -> Generator[TraceContext, None, None]:
        with self._span("sanjaya.tool_call", tool_name="vision_query", **kwargs) as ctx:
            yield ctx

    def log_final_answer(self, answer: str, iteration: int, **kwargs: Any) -> None:
        self.emit("final_answer", answer=answer[:500], iteration=iteration, **kwargs)

    def log_max_iterations_reached(self, max_iterations: int, **kwargs: Any) -> None:
        self.emit("max_iterations_reached", max_iterations=max_iterations, **kwargs)

    def log_transcription(self, **kwargs: Any) -> None:
        self.emit("transcription", **kwargs)
