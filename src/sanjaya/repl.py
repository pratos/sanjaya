...
class Sub_RLM(RLM):
    ...
    def __init__(
        ...
        fallback_model: str = "openrouter:vikhyatk/moondream2",
    ):
        """Initialize Sub_RLM.

        Args:
            model: Pydantic AI model string (e.g., 'openrouter:openai/gpt-4.1-mini')
            fallback_model: Fallback model if primary errors
        """
        self.model = model
        self.client = LLMClient(model=model, fallback_model=fallback_model)
...
class MontyREPL:
    ...
    def __init__(
        ...
        """
        self.repl = MontyRepl()
        self.sub_rlm = Sub_RLM(model=recursive_model, fallback_model=recursive_fallback_model)
