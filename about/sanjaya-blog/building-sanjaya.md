...
For longer-form tasks, the usual answer is RAG. And RAG works. Well-tuned retrieval over clean text is hard to beat for document Q&A, and modern libraries have collapsed most of the pipeline to a few lines.

Where it gets awkward is that RAG asks you to commit to a retrieval strategy up front. "Find the scoring moments in this match" and "summarize the speaker's argument" want different chunking, different ranking, different top-k. A single pipeline forces a question-agnostic shape that fits none of them perfectly.

For video, there's an additional gap: embeddings don't really reach visual content. Captioning-then-indexing catches text-in-images, but the signal of "where the camera is pointed" isn't what a caption model optimizes for. Text retrieval tops out at what the transcript says, not what the frame shows.
...
GPT-5.3-Codex as the Root LLM. GPT-4.1-mini for vision across both benchmarks, plus Moondream-2B as an additional video-frame captioner. Qwen3-30b as the critic. 10-iteration cap. $5 budget per query.
...
### 3. Use the right model for each role

I started with one model for everything. A single smart model as Root LLM, sub-LLM, captioner, and judge. That burns tokens on an expensive model doing work a cheaper one could do, and it conflates roles with different signal needs.

Sanjaya now uses four models for four jobs:

- **Root LLM** (GPT-5.3-Codex). The code writer. Decomposes the task and writes Python. Most expensive call per iteration. Needs to be strong. I tried Gemini Flash 3.1 and Arcee's Thinking model, but the results on video were meh (I suspect more prompt tuning would get them to a good place).
- **Sub-LLM** (GPT-4.1-mini). For `llm_query()` calls inside the generated code. Lots of short prompts: summarize this passage, extract this list, classify this chunk. Needs to be cheap because it runs often, not deep.
- **Captioner** (Moondream-2B via Modal). Batch-caption hundreds of frames per video. Throughput-per-dollar matters more than per-image depth. GPT-4.1-mini as a captioner also shines brighter than Moondream.
- **Critic** (Qwen3-30b). Judges answers against the schema. Has to be a different model from the Root LLM. A model grading its own output is an unreliable rubric.

The Root LLM doesn't need to extract text from a paragraph. The critic doesn't need to write code. The captioner doesn't need to reason about a whole video. Matching the model to the job is most of the cost story.
...
3. **Local models.** Moondream already runs on Modal. Next is a full local stack (local Root LLM plus local critic) so you can run Sanjaya without cloud API keys.
